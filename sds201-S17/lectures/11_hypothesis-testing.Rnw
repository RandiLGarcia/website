...
\begin{enumerate}
  ...
  \item Write \emph{one} sentence to President McCartney summarizing what you've learned about mites and wilt disease.  
  \ans
\end{enumerate}

<<eval=TRUE, message=FALSE, size='footnotesize'>>=
require(mosaic)
tally(~ outcome + treatment, data = Mites)
@

<<eval=TRUE, fig.height=4, fig.width=10, size='footnotesize', message=FALSE>>=
