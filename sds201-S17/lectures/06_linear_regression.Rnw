...
\begin{enumerate}
  \itemsep0em
  \item \href{http://www.npr.org/blogs/parallels/2014/04/02/297839429/-so-you-think-youre-smarter-than-a-cia-agent}{So You Think You're Smarter than a CIA Agent?}
...
\begin{enumerate}
  \itemsep0.6in
  \item Create a scatterplot [\cmd{qplot()}] for the $volume$ in terms of $avgtemp$
...
<<fig.width=6, fig.height=4, fig.show='hold', echo=FALSE>>=
library(broom)
mod_avg <- RailTrail %>%
  lm(volume ~ 1, data = .) %>%
  augment(RailTrail)
mod_temp <- RailTrail %>%
  lm(volume ~ avgtemp, data = .) %>%
  augment(RailTrail)
mod_data <- bind_rows(mod_avg, mod_temp) %>%
  mutate(model = rep(c("null", "slr"), each = nrow(RailTrail)))
ggplot(data = mod_data, aes(x = avgtemp, y = volume)) + 
  ...
  geom_smooth(data = filter(mod_data, model == "slr"),
              ...
              color = "dodgerblue", size = 0.5) + 
  geom_segment(aes(xend = avgtemp, yend = .fitted), 
